...
if (isset($_POST['view_data'])) {
    if ($_POST['laporan'] == 'harian') {
        ...
        $title = "Laporan Data Barang Keluar per Tanggal ".date('d/m/Y', strtotime($_POST['tanggal']));
        $_POST['bulan'] = date('Y-m');
    } else if ($_POST['laporan'] == 'bulanan') {
        $results = get_data('bulanan', date('m', strtotime($_POST['bulan'])));
        $title = "Laporan Data Barang Keluar per Bulan ".date('m/Y', strtotime($_POST['bulan']));
        $_POST['tanggal'] = date('Y-m-d');
    }
} else {
    $results = get_data('harian', date('dmy'));
    $title = "Laporan Data Barang Keluar per Tanggal ".date('d/m/Y');
}
...
function get_data($laporan, $waktu) {
    ...
    $barang_masuk = mysqli_query($conn, "SELECT * FROM barang_keluar WHERE status='finish' ORDER BY id DESC");
    foreach ($barang_masuk as $dta) {
        ...
        $pgwid = $dta['pegawai_id'];
        $pegawai = mysqli_query($conn, "SELECT * FROM pegawai WHERE id='$pgwid'");
        $pgw = mysqli_fetch_assoc($pegawai);

        if ($laporan == 'harian') {
            $tggl_masuk = date('dmy', strtotime($dta['tanggal_keluar']));
        } else if ($laporan == 'bulanan') {
            $tggl_masuk = date('m', strtotime($dta['tanggal_keluar']));
        }
                    ...
                    <tbody>
                        <?php $no=1; foreach ($results as $res) { ?>
                            <tr>
                                ...
                                <td><?= $res['nama'] ?></td>
                                <td><?= $res['nip'] ?></td>
                                <td><?= $res['jumlah_keluar'].' '.$res['satuan'] ?></td>
...
<script>
    $(document).ready(function() {
        $('#laporan-permintaan').addClass('active').parents('li').addClass('active').find('.collapse').addClass('show');
